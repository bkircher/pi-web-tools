...
=========
 ...
 -  The pi source code lives under `~/src/pi`; the Obscura source code lives
 ...
 -  When `npm audit` identifies a fix blocked by `min-release-age`, do not
    ...
    `min-release-age=0` workarounds. Report the vulnerability, the fixed
    version, and the date the fix becomes installable (publish date + cooldown
    period). Never override the `before` configuration or the `min-release-age`
    setting. The package cooldown is a deliberate supply-chain security policy.
